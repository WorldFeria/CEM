...
//
getEVisitorsReport = (async () => {

    await $.ajax({
        type: 'POST',
        url: "../php/charts.php",
        data: {
            "param": "download_visitantes",
        },
        dataType: "json",   //expect html to be returned
        ...
        success: function (response) {
            ...
            for (row in response.data['rows']) {
                ...
                Nombre = response.data['rows'][row][0]
                Apellido = response.data['rows'][row][1]
                Correo = response.data['rows'][row][2]
                Telefono = response.data['rows'][row][3]
                Rol = response.data['rows'][row][4]
                Perfil = response.data['rows'][row][5]
                Escuela = response.data['rows'][row][6]
                Pais = response.data['rows'][row][7]
            ...
            //provide the name for the CSV file to be downloaded
            hiddenElement.download = 'VisitantsDocsReport' + '.csv';
            hiddenElement.click();
...
//---------------------------------
    $(function () {
        chartExpositores = new Chart(document.getElementById("dashboard_expositores_grafica"), {
            type: 'doughnut',
            data: {
                labels: ["Confirmado","Sin confirmar","En reserva"],
                datasets: [{
                    data: [],
                    backgroundColor: customColors,
    ...
    getExpositoresData = (async (chart) => {
        ...
        await $.ajax({
            ...
            success: function (response) {
                //console.log(response);
                chart.data.datasets[0].data = [response.data['confirmado'],response.data['sinConfirmar'],response.data['reservado']];
                chart.update();
        ...
        await $.ajax({    //create an ajax request to ../php/charts.php
            ...
            success: function (response) {
                ...
                for (const property in response.data) {
                    keys.push(property);
                    values.push(response.data[property]);
                }
                for (const property in response.total) {
                    totales.push(response.total[property]);

                }


                //alert(Object.keys(response.data));
                //alert( JSON.stringify(Object.values(response.data)).substr(1,-1));

                let pabellonesEnEvento = keys;
                chart.data.labels = pabellonesEnEvento;
    ...
    getAudDocs = (async (chart) => {
        ...
        await $.ajax({    //create an ajax request to ../php/charts.php
            ...
            success: function (response) {
                ...
                let auditoriosEnEvento = keys;
                chart.data.labels = auditoriosEnEvento;
...
//--------------------------------------------------------
    ...
    $('#btn-dash-graphs-admin').click(function () {
        $('#container_dashboard_passport').collapse('hide')    
